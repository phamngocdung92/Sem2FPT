...
router.post('/', upload.single('image'), (req, res) => {
  ...
  if (!req.file) {
    ...
    if (
      ...
      //console.log(imgSrc);
      var sql =
        'INSERT INTO vietnam(image,Title,Introduce,Content)VALUES(?,?,?,?)';
...
router.get('/:id', function(req, res, next) {
  ...
  let sql = 'SELECT * FROM vietnam WHERE id = ?'    
...
router.put('/:id', upload.single('image'), (req, res) => {
  ...
  if (!req.file) {
    ...
    if (
      ...
      //console.log(imgSrc);
      var sql =
        'UPDATE vietnam SET Title=? ,Introduce=? , Content=?,image=? WHERE id=?';
...
router.delete('/:id', (req, res) => {
  let id = req.params.id;
  let sql = 'DELETE FROM vietnam WHERE id = ?';
  db.query(sql, id, (err) => {
    if (err) throw err;
